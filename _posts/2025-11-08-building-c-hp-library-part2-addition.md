...
在实现具体算法之前，我们需要确定高精度数的存储方式。

经过对比分析，我选择了 **char数组 + 字符存储** 的方案，这主要基于以下几个考量：
...
```
函数内部虽然修改了原数组，但计算完成后立即恢复，确保不影响后续运算。

这是库函数设计的**重要原则**：尽量保持数据的不可变性。
...
但是在基础代码构建时，建议先考虑核心功能的实现。

代码开发中先构建出核心架构，然后再构建维护这套架构的**"免疫系统"**
